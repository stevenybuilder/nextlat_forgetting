...
| Future-sensitive geometry | A matched-counterfactual and activation-patching pilot now answers original study 2 directly | Await a separate design freeze and compute authorization |
...
The next decision is whether to authorize the
[future-sensitive representation pilot](FUTURE_SENSITIVE_GEOMETRY_PILOT.md). It uses the archived
checkpoints, directly tests the program's second hypothesis, requires no new model training, and is
estimated at roughly $0.50–$2.50 of GPU evaluation. Nothing in this release authorizes that run.
